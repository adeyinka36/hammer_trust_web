...
function UploaderUserSearchSkeleton() {
  return (
    <section className="rounded-2xl border border-border-soft bg-white shadow-sm overflow-hidden min-h-[12rem]">
      <div className="h-24 bg-surface-tint/60 border-b border-border-soft animate-pulse" />
...
function UploaderUserSearchContent() {
  ...
  const [verifyBootstrapBusy, setVerifyBootstrapBusy] = useState(false);
  const [toast, setToast] = useState<{ message: string; tone: ToastTone } | null>(null);

  const matchedEmailParam = searchParams.get('matchedEmail');

  const dismissToast = useCallback(() => setToast(null), []);

  useEffect(() => {
    if (!matchedEmailParam) return;

    const email = decodeURIComponent(matchedEmailParam);
    const ac = new AbortController();
    setVerifyBootstrapBusy(true);

    void (async () => {
      try {
        const { user } = await searchUploaderUsers(apiClient, email, ac.signal);
        if (ac.signal.aborted) return;
        router.replace('/dashboard', { scroll: false });
        if (user) {
          setWorkspaceUser(user);
          setToast({
            message: `Workspace opened for ${user.name?.trim() || user.email}`,
            tone: 'success',
          });
        } else {
          setToast({
            message:
              'That email verified, but no matching account was found for your uploader search.',
            tone: 'info',
          });
        }
      } catch (err: unknown) {
        ...
        router.replace('/dashboard', { scroll: false });
        setToast({
          message: getErrorMessage(err, 'Could not load user workspace.'),
          tone: 'error',
        });
      } finally {
        if (!ac.signal.aborted) setVerifyBootstrapBusy(false);
      }
    })();

    return () => ac.abort();
  }, [matchedEmailParam, apiClient, router]);
  ...
  return (
    <>
      {verifyBootstrapBusy ? (
        <div
          className="fixed inset-0 z-[110] flex items-center justify-center bg-background/80 backdrop-blur-[2px] px-4"
          aria-busy
          aria-live="polite"
        >
          <div className="w-full max-w-sm rounded-2xl border border-border-soft bg-white px-8 py-10 shadow-xl text-center space-y-5">
            ...
            <div className="space-y-1">
              <p className="text-base font-bold text-foreground">Loading workspace</p>
              <p className="text-sm text-slate-600 leading-relaxed">
                Opening the matched user from verify…
              </p>
            </div>
          </div>
        </div>
...
export default function UploaderUserSearch() {
  return (
    <Suspense fallback={<UploaderUserSearchSkeleton />}>
      <UploaderUserSearchContent />
    </Suspense>
  );
}
